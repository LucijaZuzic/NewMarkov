...
for metric_name in all_metrics:
    ...
    for longlat in all_longlats:
        ...
        lines_ret[metric_name][longlat] = [ 
            ...
            [translate_method(longlat), format_e(np.quantile(series, 0.25)), format_e(np.quantile(series, 0.50)), format_e(np.quantile(series, 0.75)), format_e(max(series))],
            [translate_method(longlat), format_e(np.quantile(series, 0.80)),format_e(np.quantile(series, 0.85)), format_e(np.quantile(series, 0.90)), format_e(np.quantile(series, 0.95))],
            [translate_method(longlat), format_e(np.average(series)), format_e(np.std(series)), format_e(np.var(series)), format_e(min(series)), format_e(np.quantile(series, 0.25)), format_e(np.quantile(series, 0.50)), format_e(np.quantile(series, 0.75)), format_e(np.quantile(series, 0.80)), format_e(np.quantile(series, 0.85)), format_e(np.quantile(series, 0.90)), format_e(np.quantile(series, 0.95)), format_e(max(series))],
            [translate_method(longlat), format_e(np.average(series)), format_e(np.std(series)), format_e(np.var(series)), format_e(min(series)), format_e(np.quantile(series, 0.25)), format_e(np.quantile(series, 0.50)), format_e(np.quantile(series, 0.75)), format_e(max(series))]]
    
for metric_name in lines_ret:
    print(metric_name)
    for table_number in range(len(lines_ret[metric_name]["long no abs-lat no abs"])):
        if table_number != 1:
            continue
        strpr = "" 
...
for longlat in dict(sorted(sorty_by["euclidean"].items(), key=lambda item: item[1])): 
    ...
    for metric_name in lines_ret: 
        strpr += " & " + lines_ret[metric_name][longlat][1][2]
    strpr += " \\\\ \\hline"
    print(strpr)
