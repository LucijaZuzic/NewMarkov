...
def mosaic_one(rides, name, method_long = "", method_lat = ""):
    ...
    if method_long != "":
        plt.legend()
    if test_ride[2] + method_long + "-" + method_lat in set_drawable:
...
for name in names:
    if not os.path.isdir("mosaic/" + name + "/test"):
        os.makedirs("mosaic/" + name + "/test")
    ...
    for i in int_veh:  
        ...
        for some_file in actual_traj[subdir_name]:  
            ...
            if is_test == "test":
                for ix_longlat in range(len(all_longlats)):
                    mosaic_one([[longitudes, latitudes, subdir_name + "/cleaned_csv/" + some_file]], "mosaic/" + name + "/test/" + subdir_name + "_" + some_file.replace(".csv", "") + "_" + all_longlats[ix_longlat][0] + "_" + all_longlats[ix_longlat][1] + "_test_mosaic.png", all_longlats[ix_longlat][0], all_longlats[ix_longlat][1])
        ...
        if len(test_rides_veh):
            mosaic(test_rides_veh, "mosaic/" + name + "/" + subdir_name + "_test_mosaic.png")

        if len(train_rides_veh):
            mosaic(train_rides_veh, "mosaic/" + name + "/" + subdir_name + "_train_mosaic.png")
            
        if len(only_train_rides_veh):
            mosaic(only_train_rides_veh, "mosaic/" + name + "/" + subdir_name + "_only_train_mosaic.png")

        if len(val_rides_veh):
            mosaic(val_rides_veh, "mosaic/" + name + "/" + subdir_name + "_val_mosaic.png")

        if len(rides_veh):
            mosaic(rides_veh, "mosaic/" + name + "/" + subdir_name + "_all_mosaic.png")

        for ix_longlat in range(len(all_longlats)):

            if len(test_rides_veh):
                mosaic(test_rides_veh, "mosaic/" + name + "/" + subdir_name + "_" + all_longlats[ix_longlat][0] + "_" + all_longlats[ix_longlat][1] + "_test_mosaic.png", all_longlats[ix_longlat][0], all_longlats[ix_longlat][1])

        print(subdir_name, len(test_rides_veh), len(train_rides_veh), len(rides_veh))

    if len(test_rides_all):
        mosaic(test_rides_all, "mosaic/" + name + "/all_test_mosaic.png")

    if len(train_rides_all):
        mosaic(train_rides_all, "mosaic/" + name + "/all_train_mosaic.png")
            
    if len(only_train_rides_all):
        mosaic(only_train_rides_all, "mosaic/" + name + "/all_only_train_mosaic.png")

    if len(val_rides_all):
        mosaic(val_rides_all, "mosaic/" + name + "/all_val_mosaic.png")

    if len(rides_all):
        mosaic(rides_all, "mosaic/" + name + "/all_all_mosaic.png")

    for ix_longlat in range(len(all_longlats)):
            
        if len(test_rides_all):
            mosaic(test_rides_all, "mosaic/" + name + "/all_" + all_longlats[ix_longlat][0] + "_" + all_longlats[ix_longlat][1] + "_test_mosaic.png", all_longlats[ix_longlat][0], all_longlats[ix_longlat][1])
