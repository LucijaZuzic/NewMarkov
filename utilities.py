...
def translate_method(longlat):
    translate_name = {
        ...
        "long speed actual dir-lat speed actual dir": "Speed and heading, actual time", 
    }
...
def predict_prob_with_array(probability, probability_in_next_step, probability_in_next_next_step, array_vals, minval, maxval, stepv, isangle = False):
    ...
    for i in range(n):
        if i > 1:
            prev_prev_distance = delta_series[i - 2] # change
        if i > 0:
            prev_distance = delta_series[i - 1] # change
        if i == 0:
            distance = np.random.choice(list(probability.keys()),p=list(probability.values()))  
        if i == 1:
            if prev_distance in probability_in_next_step:
...
def predict_prob_with_array_actual(probability, probability_in_next_step, probability_in_next_next_step, array_vals, minval, maxval, stepv, isangle = False):
    roundingval = int(-np.log10(stepv))
    possible_values = (maxval - minval) // stepv + 1 
    x = []
    n = len(array_vals)
    ...
    for i in range(n):
        if i > 1:
            prev_prev_distance = array_vals[i - 2] # change
        if i > 0:
            prev_distance = array_vals[i - 1] # change
        if i == 0:
            distance = np.random.choice(list(probability.keys()),p=list(probability.values()))  
        if i == 1:
            if prev_distance in probability_in_next_step:
...
def predict_prob_with_array_short(probability, probability_in_next_step, array_vals, minval, maxval, stepv, isangle = False):
    roundingval = int(-np.log10(stepv))
    possible_values = (maxval - minval) // stepv + 1 
    x = []
    n = len(array_vals)
...
def predict_prob_with_array_short_actual(probability, probability_in_next_step, array_vals, minval, maxval, stepv, isangle = False):
    roundingval = int(-np.log10(stepv))
    possible_values = (maxval - minval) // stepv + 1 
    x = []
    n = len(array_vals)
